...
\begin{enumerate}
  ...
  %
	...
	% TODO: consider changing loss to operate on phi, rather than A....
	\item \textbf{Loss function:} $\ell \colon \Phi \times \mc{A}   \to \Real_+$.  The loss function $\ell \in \mc{L}$ scores each action output by a given decision function $\phi(z)$ with the optimal (or true) action $a$.  Standard loss functions include the $0-1$ loss and squared error loss. %Sometimes it will be convenient,  to allow $\ell$ to operate on $\Phi$, rather than $\mc{A}$. This is because each $\phi \in \Phi$ outputs an $a \in \mc{A}$, so $\ell(\phi,z)$ is simply interpretted as $\ell(a,z)$, where $a$ is the output of $\phi$.
    \item \textbf{Risk functional:} $\RR \colon \mc{L} \times \mc{P} \times \Phi \to \Real_+$.	The risk functional $\RR$ defines the criterion of optimality.  Classically, risk is defined as expected loss, $\RR_\ell(P,\phi) \defn \EE_P[\ell(\phi(z),a)]$. %More recently, risk is defined as one minus the probability that loss is smaller than some constant $\eps>0$: $\RR_\ell(P,\phi)= 1-\PP_P[ \ell(\phi(z),a) \leq \eps]$. 
\end{enumerate}
...
% TODO: simplify by removing A, redefine Z = x * y, and Phi
% TODO: R(f_t-1) = sum loss (f(z^t-1), z_t) - optimal thing
% TODO: or write it as regret of the actions explicitly, learner is implicit
% TODO: start simple least squares x,y in R

% TODO: make clear that \mh{\phi}_{t-1} obviously includes the algorithm stuff, including internal randomization
...
% \begin{align} %\nonumber
%     R(f_T) = \sum_{t=1}^T \ell(\mh{\phi}_{t-1}(z_t), a_t) - \inf_{\phi \in \Phi} \sum_{t=1}^T \ell ( \phi(z_t), a_t).
% \end{align}
% Let $\phi_*$ be the decision rule that is optimal after having seen all of the data up to $T$ in the set of decision rules under consideration ($\Phi)$, that is $\phi_*=\inf_{\phi \in \Phi} \sum_{t=1}^T \ell ( \phi, z_t)$, and simplifying the notation a bit, we have:
% \begin{align}
% R(f_T) = \sum_{t=1}^T \ell(f_{t-1}, z_t) -  \sum_{t=1}^T \ell ( \phi_*, z_t).
% \end{align}
...
% and let $s=0,\ldots, S$, $z_t \iid P_0$ denote the source measurements, and  $t=1, \ldots, T$, $z_t \iid P_1$, denote the target measurements, where 
...
\begin{align}
    ...
    \ell_1(\phi_*(z_t), a_t)  
    \right\}. 
    % \nonumber \\     &= \mh{L}^{T}(f_{1}^{T}) - L^{T}(f_*).
\end{align}
% To simplify notation, let $s=1$ correspond to $t=T_0+1$, and $S=T_0+T$, then we have:
% \begin{align}
%     R^{TL}(f_S) & = \sum_{s=1}^{S} \ell_1(\mh{\phi}_{s-1}(z_s), a_s)    -  
%     \inf_{\phi \in \Phi_1} \sum_{s=1}^{S} \ell_1(\phi(z_s), a_s).
% \end{align}
...
\begin{align}
    ...
    % \frac{\mh{L}^{T}(f_{\tr{1}}^{N}) - L^{T}(f_*)}
    % {\mh{L}^{T}(f_{\tr{S+1}}^{N}) - L^{T}(f_*)},
%     \frac{\sum_{s=1}^{S} \ell_1(\mh{\phi}_{s-1}(z_s), a_s)    -  
%     \inf_{\phi \in \Phi_1} \sum_{s=1}^{S} \ell_1(\phi(z_s), a_s)}%
%     {
% \sum_{s=1}^{S} \ell_1(\mh{\phi}_{T_0+1:s-1}(z_s), a_s)    -  
%     \inf_{\phi \in \Phi_1} \sum_{s=1}^{S} \ell_1(\phi(z_s), a_s)    }.
 \end{align}
%
...
% 
% A \emph{consistent TLM} is a TLM that asymptotically achieves Bayes risk on the target task $\mc{T}_1$.  
...
%The relationship between lifelong learning, and both online learning and transfer learning will be discussed in more detail below. 

% \emph{Online learning}~\cite{Mohri2018-tf} and \emph{transfer learning}~\cite{Pan2010-bv} can both be thought of as generalizations of learning.   
...
% Specifically, in lifelong learning, regret is defined as the excess loss on both the source and the target task, where the learning rule can use all the data from the source task for the target task:
% \begin{align}
%     \ddot{R}_S^T(f) =     \underbrace{ \frac{1}{S+T}  \sum_{s=1}^S   \ell_0(\mh{\phi}_{S+T}(z_s), a_s)   -  \ell_0({\phi}_{*}(z_s), a_s)}_{\text{source data regret}} + % \\
%      \underbrace{\frac{1}{T}   \sum_{t=1}^T  \ell_1(\mh{\phi}_{S+T}(z_t), a_t)   -  \ell_1({\phi}_{*}(z_t), a_t) }_{\text{target data regret}},
% \end{align}

% Note that lifelong regret includes the excess loss on the source task over all trials, including those trials corresponding to the target task.  This is because in lifelong learning, it is important that performance on the source target does not decrease as the learner is updating for the target task, and the loss on the source target could even continue to improve with additional target task trials (called \emph{backwards transfer}).
...
% where $w_j \propto T$ is a normalization constant designed to ensure that no task dominates over the others simply because of a scale of the loss.  Lifelong regret is simply:
% \begin{align}
%     \mh{R}_{\mv{T}}(f) = \sum_{j=1}^J \mh{R}_{\mv{T}}^j(f).
% \end{align}
...
% Thus, as in transfer learning, we consider the ratio of regret of the lifelong learner relative to the regret of a non-lifelong learner.  Define:
% \begin{align}
%     \mh{R}_{\tr{T_j}}^j(f) =  \frac{1}{w_j} \sum_{t \in \mc{T}_j}   \ell_j (\mh{\phi}_{\tr{T_j}}(z_t), a_t) - \ell_j (\phi_*(z_t),a_t), \text{ and } \mh{R}^j_{\mv{T}}(f) = \sum_{j=1}^J \mh{R}^j_{T_j}(f).
%     % }{\max_{t \in \mc{T}_j} \ell_j (\mh{\phi}_{T}(z_t), a_t) - \ell_j (\phi_*(z_t),a_t)}.
% \end{align}
...
% The only difference between supervised lifelong learning and transfer learning is that SL2 is also concerned with loss of the source task. 
% 
...
% We assume that $J$ is not provided, as that is a more difficult setting.  
In this setting,  the only difference between $\mc{T}_j$ and $\mc{T}_j'$ is $P_j \neq P_{j'}$.
% , the goal can be dramatically simplified:
...
% \end{align}
% where here and in the sequal we drop the superscript $^{L2}$ for brevity. 
% In this setting, t
...
% Note that the goal of continuously dynamic UL2 is the same as that for discrete by dynamic UL2.  The only difference is is the constraints on $P_t$:
% \begin{align} \label{eq:online}        
